...
2. /grill-with-docs
   Clarify behavior, terminology, and edge cases.

3. /to-prd
   Capture the feature as a durable spec.

4. /to-issues
   Break the spec into vertical slices.
...
7. /improve-codebase-architecture
   Use after a few slices if the code starts feeling harder to change.
```
...
```text
1. /triage
   Clarify whether the report is actionable.

2. /diagnose
   Reproduce the bug and identify the cause.

3. /tdd
   Add a regression test and fix the bug.

4. /improve-codebase-architecture
   Use only if the bug exposed bad seams or poor design.
```
...
```text
1. /zoom-out
   Understand the current module boundaries.

2. /improve-codebase-architecture
   Identify deep-module opportunities and better seams.

3. /tdd
   Add safety coverage around current behavior.

4. /tdd
   Refactor incrementally while keeping behavior stable.
```
